...
category: "pwn - medium"
tags: [x86_64, automotive, "signess bug", oob]
date: 2026-09-09
description: "my solution for the fcsc 2026 autodiag pwn challenge which calls us to exploit an automotive system"
---
...
it looks like this server communicates with packets that must have the following header: 
...
opcode numbers to execute specific commands:
...
payload (64 bit)
```
now we have the final packet form:
```
[32-bit] magic
[32-bit] version
[32-bit] op
...
static int op_resp(struct ServerState *st, int client_fd, const struct MsgHdr *hdr,
    ...
    idx = (int64_t)conn_id; <---------- conn_id is casted as a signed integer
    if (idx > MAX_CONN_SLOTS - 1) { <---------------- there is no lower bounds check on our idx
        return send_response(client_fd, hdr->op, hdr->req_id, ST_INVALID, NULL, 0);
    }
    
    target_fd = st->conn_fds[idx]; <------- and then the user controlled idx (through conn_id) is parsed as the target fd
...
```
this run_update function looks interesting as well:
```
...
after looking through *ivi_update_runner.cpp* further we constructed an update file format that we later utilize to build a valid but malicious update payload*(update.bin)*:
```
[0] UPD0: magic
...
$$M \equiv 0^e \pmod n$$  
which equals 0.
the server converts that 0 back into a byte array to use as the symmetric AES key for the rest of the file *(update.bin)*. `S = 0` which translates to an AES key of `\x00 * 256`
...
io.interactive()
```
All the following functions are used to to make a valid update that decrypts gracefully  based on *ivi_update_runner.cpp* operations on our update payload (we build a counterfeit *update.bin*)
